...
kernel_auction_match = '''
extern "C" __global__ void kernel_auction_match(int b,int n,const float * __restrict__ xyz1,const float * __restrict__ xyz2,int * matchl,int * matchr, float * cost)
{
...
class AuctionMatchFunction(torch.autograd.Function):
    @staticmethod
    def forward(self, xyz1, xyz2):
        ...
        cost = torch.cuda.FloatTensor(B, N, N).fill_(0)
        # cost = torch.cuda.FloatTensor(B, N, N)

        if xyz1.is_cuda == True:
            cupy_launch('kernel_auction_match', cupy_kernel('kernel_auction_match', {
                'b': B,
                'n': N,
                'xyz1': xyz1,
                'xyz2': xyz2,
                ...
                grid=tuple([ 32, 1, 1 ]),
                block=tuple([ 512, 1, 1 ]),
                args=[ B, N, xyz1.data_ptr(), xyz2.data_ptr(), match_left.data_ptr(), match_right.data_ptr(), cost.data_ptr() ]
            )

            

        # auc_mat.auction_match_cuda(B, N, xyz1, xyz2, match_left, match_right, temp)

        return match_left, match_right

    @staticmethod
    def backward(self, ml, mr, a=None):
        return None, None

class AuctionMatch(torch.nn.Module):
    def __init__(self):
        super(AuctionMatch, self).__init__()

    def forward(self, xyz1, xyz2):
        return AuctionMatchFunction.apply(xyz1, xyz2)